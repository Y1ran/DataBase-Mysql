...
import numpy as np
import ABR_TP as ABR
def test(user_id):
    # path setting
    TRAIN_TRACES = './final_network_trace/origin/'
#    Qiang = './new_network/qiangwang/'
#    Zhong = './new_network/zhongwang/'
#    Ruo = './new_network/ruowang/'
    #train trace path setting,
...
#            print("rebuf:",rebuf_time)
#            print("S_frame",S_frame_type)
        ...
        if end_of_video:
            ...
            if video_count >= len(all_file_names):
                break
            cnt = 0
            print("change_number", abr.changenumber)
            print("end_delay", abr.delay)
            print("freezetime", abr.freezetime)
            reward_all = 0
            last_bit_rate = 0
            bit_rate = 0
            target_buffer = 1.5
#            print(abr.TP_list, abr.Buffer_list)
#            file_handle1 = open('TP_list.txt',mode='w')
#            for tp in abr.TP_list:
#                file_handle1.write(str(tp))

#                file_handle1.write('\n')
            writeList2CSV(abr.buffer_list, './TP_list.csv')
#            file_handle2 = open('Buffer_list.txt',mode='w')
#            for buffer in abr.TP_list:
#                file_handle2.write(str(buffer))
#                # 写入换行符
#                file_handle2.write('\n')
#            file_handle1.close()
#            file_handle2.close()
